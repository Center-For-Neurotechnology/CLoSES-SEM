function [sumCoherencePerChannel, avCoherence, coherenceValue, pairChannels] = runCoherence_EML(filteredData, lowFreq, highFreq, Fs, detectChannelInds,overlapSamples)
%#codegen
...
filteredDataSlow = downsample(filteredData,downSamplingBy); %round(Fs/(highFreq*10))); % keep 10 times highest frequency (could be less)
maxLen = floor(size(filteredData,1)/downSamplingBy);
filteredDataSlow = filteredDataSlow(1:maxLen,:);
