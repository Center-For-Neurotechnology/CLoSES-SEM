function [coherenceValue] = runCoherence_Downsampled_EML(filteredData, lowFreq, highFreq, Fs, detectPairInds, overlapSamples)
%#codegen
...
filteredDataDownSampled = downsample(filteredData,downSamplingBy); %round(Fs/(highFreq*10))); % keep 10 times highest frequency (could be less)
maxLen = floor(size(filteredData,1)/downSamplingBy);
filteredDataToUse = filteredDataDownSampled(1:maxLen,:);
...
%Initialize variables (needed for Simulink real-time)
    ...
    cmethod = @plus;
    [PxxTemp, w] = localComputeSpectra(Sxx,filteredDataToUse(:,detectChannInds),[],xStart,xEnd,win,options,'mscohere',k,cmethod);
    ...
    for iPair=1:nPairs % Only compute pxy for paris of values!
        if ~isempty(intersect(detectPairInds, iPair)) %Only compute for those pairs that will be used for detection!
            ...
            Pxy = localComputeSpectra(Sxy, filteredDataToUse(:,indCh1),filteredDataToUse(:,indCh2), xStart, xEnd, win, options,'mscohere',k,cmethod);
            coherValPerPair = (abs(Pxy).^2)./bsxfun(@times,Pxx(:,indCh1),Pxx(:,indCh2)); % Cxy
