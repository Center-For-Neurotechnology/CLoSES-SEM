...
switch upper(featureName)
    ...
    case 'VARIANCEOFPOWER'
        ...
        variantConfig.WHICH_FEATURE_BASELINE = 4;
        variantConfig.WHICH_DETECTOR = 4;
        nFeatures = sCoreParams.decoders.txDetector.nFilteredChannels; % To have correct number for visualization! - RIZ: it should be improved!        case 'VARIANCEOFPOWER'
    case 'COHERENCE'
        variantConfig.WHICH_FEATURE = 5;
        variantConfig.WHICH_FEATURE_BASELINE = 5;
        variantConfig.WHICH_DETECTOR = 8; %5; - changed to neural model
        nFeatures = sCoreParams.decoders.txDetector.nPairs;
    case 'CORRELATION'
        variantConfig.WHICH_FEATURE = 6;
        variantConfig.WHICH_FEATURE_BASELINE = 6;
        variantConfig.WHICH_DETECTOR = 5;
        nFeatures = sCoreParams.decoders.txDetector.nPairs;
    case 'IED'
...
if nFeatures ~= sCoreParams.decoders.txDetector.nFeatures
    sCoreParams.decoders.txDetector.nFeatures = nFeatures;
