...
%nspTime = 1;
sInputDataOdd=[];sInputTriggerOdd=[];sInputDataEven=[];sInputTriggerEven=[];
switch upper(whatTypeSimulation)
    ...
    case {'REAL','NEV'} % These are MAT files created from NEV or Plexon data - in referential montge, so we assume we can use consecutive channels for bipolar
        ...
        sInputTrigger.time=[];
        sInputTrigger.signals.values = zeros(size(sInputData.signals.values,1),2);
        sInputTrigger.signals.values(:,1) = stRealDataSim.triggerVals';
        sInputTrigger.signals.dimensions=2;
...
% Divide Simulated data on Even and Odd
% sInputDataOdd = sInputData;
% sInputDataOdd.signals.values = sInputData.signals.values(1:2:end,:);
% sInputDataEven = sInputData;
% sInputDataEven.signals.values = sInputData.signals.values(2:2:end,:);
% sInputTriggerOdd = sInputTrigger;
% sInputTriggerOdd.signals.values = sInputTrigger.signals.values(1:2:end,:);
% sInputTriggerEven = sInputTrigger;
% sInputTriggerEven.signals.values = sInputTrigger.signals.values(2:2:end,:);
