function varargout = InitDialogNeuralDecoder(varargin)
% InitDialogNeuralDecoder MATLAB code for InitDialogNeuralDecoder.fig
%      InitDialogNeuralDecoder by itself, creates a new InitDialogNeuralDecoder or raises the
%      existing singleton*.
%
%      H = InitDialogNeuralDecoder returns the handle to a new InitDialogNeuralDecoder or the handle to
%      the existing singleton*.
%
%      InitDialogNeuralDecoder('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in InitDialogNeuralDecoder.M with the given input arguments.
%
%      InitDialogNeuralDecoder('Property','Value',...) creates a new InitDialogNeuralDecoder or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before InitDialogNeuralDecoder_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to InitDialogNeuralDecoder_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help InitDialogNeuralDecoder

% Last Modified by GUIDE v2.5 17-Nov-2017 12:35:08

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @InitDialogNeuralDecoder_OpeningFcn, ...
                   'gui_OutputFcn',  @InitDialogNeuralDecoder_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before InitDialogNeuralDecoder is made visible.
function InitDialogNeuralDecoder_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to InitDialogNeuralDecoder (see VARARGIN)

    % Choose default command line output for InitDialogNeuralDecoder
    handles.output = 'No';
    %handles.txtModelFileName = '';
    handles.experimentType = 'MSIT';
...
%          );
    ...
    % UIWAIT makes InitDialogNeuralDecoder wait for user response (see UIRESUME)
    uiwait(handles.figure1);
end

% --- Outputs from this function are returned to the command line.
function varargout = InitDialogNeuralDecoder_OutputFcn(hObject, eventdata, handles)
...
function btnClosedLoop_Callback(hObject, eventdata, handles)
    ...
    if isempty(get(handles.txtModelFileName,'String'))
        hWarnDlg = warndlg({'Select SIMULINK model file (usually on.\CompileFiles)','Otherwise system will start with Default'},' Missing Simulink Model File!');
        return;
    end
    if isempty(get(handles.txtNeuralModelFileName,'String'))
        hWarnDlg = warndlg({'Select NEURAL decoder file','Otherwise system will Crash!'},' Missing Neural Decoder File!');
        return;
    end
...
function txtNeuralModelFileName_Callback(hObject, eventdata, handles)
% hObject    handle to txtNeuralModelFileName (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of txtNeuralModelFileName as text
%        str2double(get(hObject,'String')) returns contents of txtNeuralModelFileName as a double
end

% --- Executes during object creation, after setting all properties.
% function txtNeuralModelFileName_CreateFcn(hObject, eventdata, handles)
% % hObject    handle to txtNeuralModelFileName (see GCBO)
% % eventdata  reserved - to be defined in a future version of MATLAB
% % handles    empty - handles not created until after all CreateFcns called
% 
...
% --- Executes on button press in btnNeuralModelFile.
function btnNeuralModelFile_Callback(hObject, eventdata, handles)
% hObject    handle to btnNeuralModelFile (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
    [filename, pathname] = uigetfile({'*.mat'},'Select Neural Model File (model.mat)');
    if (length(filename)>1)
        neuralModelFileName = [pathname, filename];
        set(handles.txtNeuralModelFileName, 'String', neuralModelFileName);
        guidata(hObject, handles);
    end
end
