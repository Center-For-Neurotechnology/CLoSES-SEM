function TL = ay_individual_decoder(model,Param,Xs,valid,Yk,Yprv)
...
for m=1:Ms
    if valid(m,1)
        ...
        if size(xTProb,1) > 1
            TProb(m,:) = squeeze(sum(xTProb));
        else
            TProb(m,:) = squeeze(xTProb);
        end
        TProb(m,:) = TProb(fill_ind,:)/sum(TProb(m,:)); 
    end
end
