function [XPos,Estimate,TProb]= ay_one_step_decoder_realTime(modelType, Param_W, Param_Dispersion, Param_EncModel_XPow, XPre, TransP, Xs, valid, Yk, Yprv)
...
% YK is the input
const_scale = 13.8155; % =log(1e6) but removes computing log each time;
%max_mdl_b   = 10000;
%tic
...
%             end
            ...
            InX  = zeros(lTime,2); % RIZ make sure that it is always 2! if model is different it shuild be changed!!
            fill_ind =0;
            for i=1:length(Xmdl)
                if Xmdl(i)==1
                    fill_ind = fill_ind + 1;
...
%                       mdl_a    = 1/tParam_Dispersion;  % Changed to 1/tParam_Dispersion in place of tParam_Dispersion
...
%             end
 %           if modelType==2     % Normal
                % current step
                In       = [InX repmat(tAux,lTime,1)];
                if sum(isinf(In))==0 & sum(isnan(In))==0
                    if isinf(Yk(m))==0 & isnan(Yk(m))==0
                        mdl_mean = In * tParam_W;
                        mdl_std  = sqrt(tParam_Dispersion);
                        xTProb(ws,:)  = pdf('normal',Yk(m),mdl_mean,repmat(mdl_std,lTime,1));
                    end
               end
     %       end
    %    end
